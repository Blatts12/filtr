defmodule Pex.ValidatorTest do
  ...
  describe "string validation" do
    test "min length validation" do
      ...
      assert Validator.run("hi", :string, [min: 3]) == {:error, ["must be at least 3 characters long"]}
      assert Validator.run("abc", :string, [min: 3]) == {:ok, "abc"}
    end

    test "max length validation" do
      assert Validator.run("hi", :string, [max: 5]) == {:ok, "hi"}
      assert Validator.run("hello world", :string, [max: 5]) == {:error, ["must be at most 5 characters long"]}
      assert Validator.run("hello", :string, [max: 5]) == {:ok, "hello"}
    end

    test "pattern validation" do
      email_pattern = ~r/@/
      assert Validator.run("test@example.com", :string, [pattern: email_pattern]) == {:ok, "test@example.com"}
      assert Validator.run("invalid-email", :string, [pattern: email_pattern]) == {:error, ["does not match pattern"]}
    end

    test "starts_with validation" do
      assert Validator.run("hello world", :string, [starts_with: "hello"]) == {:ok, "hello world"}
      assert Validator.run("hi world", :string, [starts_with: "hello"]) == {:error, ["does not start with hello"]}
    end

    test "ends_with validation" do
      assert Validator.run("hello world", :string, [ends_with: "world"]) == {:ok, "hello world"}
      assert Validator.run("hello universe", :string, [ends_with: "world"]) == {:error, ["does not end with world"]}
    end
    ...
    test "multiple string validations" do
      ...
      assert Validator.run("test@example.com", :string, opts) == {:error, ["must be at most 10 characters long"]}
      assert Validator.run("a@b", :string, opts) == {:error, ["must be at least 5 characters long"]}
  ...
  describe "date validation" do
    test "min date validation" do
      ...
      assert Validator.run(~D[2019-12-31], :date, [min: min_date]) == {:error, ["must be after or equal to 2020-01-01"]}
    ...
    test "max date validation" do
      ...
      assert Validator.run(~D[2026-01-01], :date, [max: max_date]) == {:error, ["must be before or equal to 2025-12-31"]}
  ...
  describe "datetime validation" do
    test "min datetime validation" do
      ...
      assert Validator.run(~U[2023-06-15 12:00:00Z], :datetime, [min: min_datetime]) == {:ok, ~U[2023-06-15 12:00:00Z]}
      assert Validator.run(~U[2019-12-31 23:59:59Z], :datetime, [min: min_datetime]) == {:error, ["must be after or equal to 2020-01-01 00:00:00Z"]}
      assert Validator.run(~U[2020-01-01 00:00:00Z], :datetime, [min: min_datetime]) == {:ok, ~U[2020-01-01 00:00:00Z]}
    end

    test "max datetime validation" do
      max_datetime = ~U[2025-12-31 23:59:59Z]
      assert Validator.run(~U[2023-06-15 12:00:00Z], :datetime, [max: max_datetime]) == {:ok, ~U[2023-06-15 12:00:00Z]}
      assert Validator.run(~U[2026-01-01 00:00:00Z], :datetime, [max: max_datetime]) == {:error, ["must be before or equal to 2025-12-31 23:59:59Z"]}
      assert Validator.run(~U[2025-12-31 23:59:59Z], :datetime, [max: max_datetime]) == {:ok, ~U[2025-12-31 23:59:59Z]}
    end

    test "in validation for datetimes" do
      options = [~U[2023-01-01 00:00:00Z], ~U[2023-06-15 12:00:00Z], ~U[2023-12-31 23:59:59Z]]
      assert Validator.run(~U[2023-06-15 12:00:00Z], :datetime, [in: options]) == {:ok, ~U[2023-06-15 12:00:00Z]}
      assert Validator.run(~U[2023-07-01 12:00:00Z], :datetime, [in: options]) == {:error, ["value not in list"]}
    end
  end
  ...
  describe "list validation" do
    ...
    test "max length validation" do
      ...
      assert Validator.run(["a", "b", "c", "d"], :list, [max: 3]) == {:error, ["must be at most 3 items long"]}
    ...
    test "in validation not supported for lists - options are filtered out" do
      ...
      assert Validator.run(["purple", "yellow"], :list, [in: allowed]) == {:ok, ["purple", "yellow"]}
    end
  end

  describe "custom validation" do
    test "custom validation function returning :ok" do
      validator = fn value -> if String.length(value) > 5, do: :ok, else: {:error, "too short"} end
    ...
    test "custom validation function returning false" do
      ...
      assert Validator.run("test", :string, [validate: validator]) == {:error, ["validation failed"]}
    end
    ...
    test "combines custom validation with other validations" do
      ...
      assert Validator.run("a@b.c", :string, opts) == {:error, ["must be at least 6 characters long"]}
      assert Validator.run("test@example.com", :string, opts) == {:ok, "test@example.com"}
      assert Validator.run("invalid", :string, opts) == {:error, ["must be at least 6 characters long", "invalid email"]}
    end
  end

  describe "multiple validation errors" do
    test "collects multiple validation errors" do
      opts = [min: 10, max: 5]  # Impossible constraint to trigger both errors
